      ...
      <Container>
        ...
        <section id="catalog-results" className={`${hasActiveFilters ? "" : "mt-16 "}scroll-mt-28`}>
          <SectionTitle
            ...
            description={
              hasActiveFilters
                ? "Список обновляется по выбранной категории и поисковому запросу. В карточках сразу видно, какие позиции готовы к прямому сценарию, какие остаются marketplace-first, а какие идут только через запрос."
                : "Карточки сразу показывают цену, текущий сценарий продажи и следующий шаг: прямая покупка без корзины, переход в marketplace-сценарий или запрос по задаче."
            }
          />
